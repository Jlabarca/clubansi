...
function initPostFullContentHeight() {
    $(".blog-posts .post-full").each(function() {
        if ($(this).find(".post-image").css("float") == "left") {
            var imgHeight = $(this).find(".post-image").height();
            $(this).find(".post-content").css("min-height", imgHeight);
        }
    });
    $(".products.layout-list .product").each(function() {
        if ($(this).find(".product-image").css("float") == "left") {
            var imgHeight = $(this).find(".product-image").outerHeight();
            $(this).find(".product-content").css("min-height", imgHeight - $(this).find(".product-action").outerHeight() - 1);
        }
    });
}
